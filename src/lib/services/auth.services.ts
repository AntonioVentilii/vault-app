import { II_MAX_TIME_TO_LIVE_NS, II_WINDOW_FEATURES } from '$lib/constants/auth.constants';
import { AuthClientProvider } from '$lib/providers/auth-client.providers';
import {
	clearDevIdentity,
	createDevIdentity,
	loadDevIdentity
} from '$lib/services/dev-identity.services';
 ...
 * origin — the same thing the Juno SDK obtained, from the same provider
 * (https://id.ai, the `@icp-sdk/auth` default) — so the principal each user
 * gets is unchanged.
 */
 ...
 * Juno's own Google sign-in — and it needs no client ID of our own, which is
 * why the old Google button was permanently hidden behind an unset
 * `GOOGLE_CLIENT_ID`.
 */
...
export const initAuth = async (): Promise<void> => {
	...
	if (nonNullish(devIdentity)) {
		userStore.set(toUser(devIdentity));
		return;
	}
